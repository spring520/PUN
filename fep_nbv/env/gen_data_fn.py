...
def shapenet_full(num_azimuth=8,num_elevation=5,radius=[2.7319998741149902]):
...
def shapenet_eval(num_azimuth=8,num_elevation=6,radius=[2.7319998741149902]):
    trajs = [] # pose_point_to(loc=point*scale, target=target*scale, up=np.random.rand(3)) 旋转矩阵的列表
    azimuths = np.linspace(0, 2 * np.pi, num=num_azimuth, endpoint=False)
...
def shapenet_init(n=1):
    ...
    radius = 2.7319998741149902 * torch.ones((n))
...
if __name__=='__main__':
    ...
    for elev in elevations:
        for azim in azimuths:
            x = radius_val * np.sin(elev) * np.cos(azim)
            y = radius_val * np.sin(elev) * np.sin(azim)
            z = radius_val * np.cos(elev)
            points.append([x, y, z])
