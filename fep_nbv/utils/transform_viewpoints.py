...
def xyz2polar(x,y,z):
    # 计算方位角 (Azimuth) 范围 [0, 2π]
    azimuth = np.arctan2(y, x)  # 计算范围 [-π, π]
    azimuth = np.mod(azimuth, 2 * np.pi)  # 确保在 [0, 2π] 范围内

    # 计算俯仰角 (Elevation) 范围 [0, π]
    r = np.sqrt(x**2 + y**2 + z**2)  # 计算半径
    elevation = np.arccos(z / r)  # 计算范围 [0, π]
