...
class NGPModel(Model):
    ...
    def populate_modules(self):
        ...
        self.field = NerfactoField(
            aabb=self.scene_box.aabb, # tensor[[-2,-2,-2],[2,2,2]]
            num_images=self.num_train_data, # 401
            log2_hashmap_size=self.config.log2_hashmap_size, # 19
            max_res=self.config.max_res, # 512
            spatial_distortion=scene_contraction, # None
        ...
        self.sampler = VolumetricSampler(
            occupancy_grid=self.occupancy_grid, # self.occupancy_grid
            density_fn=self.field.density_fn, # <bound method Field.density_fn of NerfactoField(
        )
        ...
        self.renderer_geometry_entropy = WeightDistributionEntropyRenderer()
        self.renderer_rgb_entropy = RGBVarianceEntropyRenderer()
    ...
    def get_combine_entropy(self, ray_bundle: RayBundle):
        assert self.field is not None
        num_rays = len(ray_bundle)
        ...
        geometry_entropy = self.renderer_geometry_entropy(
            field_outputs = field_outputs,
            ray_samples=ray_samples, 
            ray_indices=ray_indices, 
            num_rays=num_rays
        )
        rgb_entropy = self.renderer_rgb_entropy(
            field_outputs = field_outputs,
            ray_samples=ray_samples, 
            ray_indices=ray_indices, 
            num_rays=num_rays
        ...
        return entropy,geometry_entropy,rgb_entropy
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        field_outputs = self.field(ray_samples) # 是什么东西
    ...
    def get_metrics_dict(self, outputs, batch):
        image = batch["image"].to(self.device) # batch x rgbd
        ...
        metrics_dict["psnr"] = self.psnr(predicted_rgb, gt_rgb) # max 1
