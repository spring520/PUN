...
class HEALPixSampler(BaseSampler):
    def __init__(self, cfg=None):
        super().__init__(cfg)
    ...
    def sample(self, n_sample, *args, **kwargs):
        ...
        if self.pipeline is not None and self.cfg.check_density:
            ...
            result = sample_poses_HEALPix(n_side=2,radius=self.radius)
            ...
            return sample_poses_HEALPix(n_side=2)
...
class SphericalSampler(BaseSampler):
    ...
    def sample(self, n_sample, *args, **kwargs):
        ...
        result = sample_poses_spherical(n_sample,radius=self.radius) 
...
def sample_poses_spherical(n_sample,radius=2.7319998741149902):
...
def sample_poses_HEALPix(n_side=2,radius=None):
    poses = generate_HEALPix_viewpoints(n_side=n_side,radius=radius)
    return poses
...
if __name__=='__main__':
    poses = sample_poses_spherical(100)
    print(poses.shape)