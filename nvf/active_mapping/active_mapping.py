...
class ActiveMapper:
    ...
    def initialize_config(self, config_home: str, dataset_path: str, model: Literal["instant-ngp", "nerfacto"] = "instant-ngp") -> str:
        ...
        self.trainer = self.config.setup(local_rank=0, world_size=1, init_step = init_step) # ini
        self.trainer.setup()
    ...
    def add_image(
        ...
        if model_option == "load_ckpt": # 没有用到这部分内容
        ...
        for i in range(len(poses)):
            transform = to_transform(poses[i]) # 从四元数+平移向量转换为变换矩阵
            transform_poses.append(transform)
            normalize_images.append(images[i].astype("float32") / 255.0)

        self.trainer.update_data(images=normalize_images, poses=transform_poses) # 干了什么
        ...
        t1 = time.time()
        self.trainer.train()
        print(f'Self.trainer.train duration: {time.time()-t1:.4f} seconds')
    ...
    def get_cost(
        ...
        for batch in range(bs):
            ...
            if self.baseline == 'nvf':
                entropy = get_entropy_for_camera_ray_bundle(pipeline.model, camera_ray_bundle)
            else:
                entropy,geometry_entropy,rgb_entropy = get_entropy_for_camera_ray_bundle_combine(pipeline.model, camera_ray_bundle)
            # print(f'{entropy.min()} {entropy.max()} {geometry_entropy.min()} {geometry_entropy.max()}')
            # entropy = 1*entropy # +0*geometry_entropy
            if self.baseline == 'nvf':
                entropy = entropy
            elif self.baseline == 'WD':
                entropy = geometry_entropy
            elif self.baseline == 'activenerf':
                entropy = rgb_entropy
            else:
