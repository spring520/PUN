...
@torch.no_grad()
def get_entropy_for_camera_ray_bundle_combine(model, camera_ray_bundle):
        num_rays_per_chunk = model.config.eval_num_rays_per_chunk
        image_height, image_width = camera_ray_bundle.origins.shape[:2]
        num_rays = len(camera_ray_bundle)
        entropy_list = []
        geometry_entropy_list = []
        rgb_entropy_list = []
        for i in range(0, num_rays, num_rays_per_chunk):
            start_idx = i
            end_idx = i + num_rays_per_chunk
            ray_bundle = camera_ray_bundle.get_row_major_sliced_ray_bundle(start_idx, end_idx)
            outputs,outputs_geometry,outputs_rgb = model.get_combine_entropy(ray_bundle=ray_bundle)
            # for output_name, output in outputs.items():  # type: ignore
            #     if not torch.is_tensor(output):
            #         # TODO: handle lists of tensors as well
            #         continue
            entropy_list.append(outputs)
            geometry_entropy_list.append(outputs_geometry)
            rgb_entropy_list.append(outputs_rgb)

        entropy = torch.cat(entropy_list).view(image_height, image_width, -1)  # type: ignore
        geometry_entropy = torch.cat(geometry_entropy_list).view(image_height, image_width, -1)  # type: ignore
        rgb_entropy = torch.cat(rgb_entropy_list).view(image_height, image_width, -1)  # type: ignore
        return entropy,geometry_entropy,rgb_entropy

@torch.no_grad()
def get_entropy_for_ray_bundle_uniform(model, ray_bundle_uniform):
        num_rays_per_chunk = model.config.eval_num_rays_per_chunk
        image_height, image_width = ray_bundle_uniform.origins.shape[:2]
        num_rays = len(ray_bundle_uniform)
        entropy_list = []
        for i in range(0, num_rays, num_rays_per_chunk):
            start_idx = i
            end_idx = i + num_rays_per_chunk
            ray_bundle = ray_bundle_uniform.get_row_major_sliced_ray_bundle(start_idx, end_idx)
            outputs = model.get_entropy(ray_bundle=ray_bundle)
            # for output_name, output in outputs.items():  # type: ignore
            #     if not torch.is_tensor(output):
            #         # TODO: handle lists of tensors as well
