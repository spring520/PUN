...
def shapenet_full(num_azimuth=8,num_elevation=8,radius=[2]):
    ...
    trajs = torch.concat((rot_quat[:,[1,2,3,0]], cameras),dim=1)
    
    return trajs

def shapenet_eval(num_azimuth=8,num_elevation=8,radius=[2]):
    trajs = [] # pose_point_to(loc=point*scale, target=target*scale, up=np.random.rand(3)) 旋转矩阵的列表
    azimuths = np.linspace(0, 2 * np.pi, num=num_azimuth, endpoint=False)
    elevations = np.linspace(0, np.pi, num=num_elevation)
    radius = np.asarray(radius)
...
def nerf_full(num_azimuth=8,num_elevation=5,radius=[2.7319998741149902]):
    trajs = [] # pose_point_to(loc=point*scale, target=target*scale, up=np.random.rand(3)) 旋转矩阵的列表
    azimuths = np.linspace(0, 2 * np.pi, num=num_azimuth, endpoint=False)
    elevations = np.linspace(0, np.pi, num=num_elevation)
    radius = np.asarray(radius)
...
def nerf_eval(num_azimuth=8,num_elevation=6,radius=[2.7319998741149902]):
    trajs = [] # pose_point_to(loc=point*scale, target=target*scale, up=np.random.rand(3)) 旋转矩阵的列表
    azimuths = np.linspace(0, 2 * np.pi, num=num_azimuth, endpoint=False)
