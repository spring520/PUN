...
class RefMetricTracker(object):
    ...
    def __init__(self, config=None, env=None):
        ...
        if self.ref_images.max()>1.1:
            self.ref_images /= 255.
        self.ref_poses = torch.stack(transforms) # 20x4x4
        ...
        self.base_log_dir = config.exp_folder
    ...
    def update(self, pipeline, pose, step):
        ...
        with torch.no_grad():
            ...
            print('eval', {k:round(self.metric_hist[k][-1],4) for k in ['psnr','ssim','lpips','rgb_loss', 'acc', 'comp','cr','vis', 'vis_area']})
            # print('eval', {k:round(self.metric_hist[k][-1],4) for k in ['psnr', 'rgb_loss']})
            for pp in pose:
                self.visibility_metric.update(pp[None,...])
    ...
    def visualize(self, pipeline):
        ...
        for i in range(n_cameras):
            ...
            camera_to_worlds=self.ref_poses[i,:-1,:] # 3x4
