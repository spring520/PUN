...
def Uncertainty_distribution_calculation(cfg, NeRF_pipeline, absolute_viewpoint_poses, gt_images, input_viewpoint_index):
    Uncertainty = {'PSNR':[],"uncertainty":[],'SSIM':[],"MSE":[],'acc':[],'comp':[],'cr':[],'vis':[],'vis_area':[]}

    env = set_env(cfg)
    geometry_metric = GeometryMeshMetric(scene=env.scene, num_sample_points=500000)
    ...
    with torch.no_grad():
        for i in range(len(transforms)):
            visibility_metric = VisibilityMeshMetric(scene=env.scene)
            visibility_metric.update(absolute_viewpoint_poses[None,0])
            visibility_metric.update(absolute_viewpoint_poses[None,i])
            cameras = Cameras(
                fx=fx,
                fy=fy,
                cx=cx,
            ...
            # 1. 先计算某个视角下的点云
            # 2. 计算CR等指标
            accuracy, completion, completion_ratio = geometry_metric.compute_pcl_metric(NeRF_pipeline.trainer.pipeline)
            visibility, vis_area = visibility_metric.get_value()
            ...
            Uncertainty['acc'].append(psnr.item())
            Uncertainty['comp'].append(psnr.item())
            Uncertainty['cr'].append(psnr.item())
            Uncertainty['vis'].append(psnr.item())
            Uncertainty['vis_area'].append(psnr.item())
...
def generate_distribution_single_rotation_with_geometry(model_path,input_viewpoint_index,offset_phi_index,cfg):
...
if __name__=='__main__':
    ...
    generate_distribution_single_rotation_with_geometry(model_path,input_viewpoint_index,offset_phi_index,cfg)
